...
def insert():
	...
	if(id=="" or name=="" or phone==""):
		...
		con = mysql.connect(host="localhost", user="root", password="anik1234", database="test")
		cursor = con.cursor()
		cursor.execute("insert into bankdata values('"+id+"','"+name+"','"+phone+"','"+password+"','"+amount+"')")
...
def update():
	...
	if(id=="" or name=="" or phone=="" or amount==""):
		MessageBox.showinfo("Update Status", "All fields are required except Password")
	else:
		con = mysql.connect(host="localhost", user="root", password="anik1234", database="test")
		cursor = con.cursor()
		cursor.execute("update bankdata set name='"+name+"',phone='"+phone +"',amount='"+amount +"' where id='"+id+"'")
		cursor.execute("commit")
...
def user_update():
	...
	if(phone=="" or password==""):
		...
		con = mysql.connect(host="localhost", user="root", password="anik1234", database="test")
		cursor = con.cursor()
		cursor.execute("update bankdata set password='"+password+"',phone='"+phone +"' where id='"+userid+"'")
		cursor.execute("commit")
...
def pg2_show():
	...
	for row in rows:
		insertData = 'ID:' + str(row[0]) + '  Name:'+ row[1]+ '  Phone:'+ row[2] + '  Amount:'+ str(row[4])
...
def upg2_show():
	...
	for row in rows:
		if(userid == str(row[0])):
			...
			upg2_amount2 = Label(user_page_2,text=str(row[4]) ,font=('bold',20))
...
pg0_button = Button(page0, text='USER', font=('Arial', 20, 'bold'), command=lambda: show_frame(user_page_1))
pg0_button.place(x=100, y=90)

pg0_button = Button(page0, text='ADMIN', font=('Arial', 20, 'bold'), command=lambda: show_frame(admin_page_1))
...
pg1_button = Button(admin_page_1, text='LOGIN', font=('Arial', 13, 'bold'), command=admin_check)
...
upg1_button = Button(user_page_1, text='LOGIN', font=('Arial', 13, 'bold'), command=user_check)
...
pg2_insert = Button(admin_page_2, text='Insert', font=("italic",10),bg="white",command=insert)
pg2_insert.place(x=20,y=200)

pg2_delete = Button(admin_page_2, text='Delete', font=("italic",10),bg="white",command=delete)
pg2_delete.place(x=70,y=200)

pg2_update = Button(admin_page_2, text='Update', font=("italic",10),bg="white",command=update)
pg2_update.place(x=130,y=200)

pg2_get = Button(admin_page_2, text='Get', font=("italic",10),bg="white",command=get)
pg2_get.place(x=190,y=200)

pg2_logout = Button(admin_page_2, text='Logout', font=("italic",10),bg="white",command=lambda: show_frame(page0))
...
upg2_toup = Label(user_page_2,text='To update your password and phone number:',font=('bold',10))
...
upg2_update = Button(user_page_2, text='Update', font=("italic",10),bg="white",command=user_update)
upg2_update.place(x=130,y=200)

upg2_logout = Button(user_page_2, text='Logout', font=("italic",10),bg="white",command=lambda: show_frame(page0))
